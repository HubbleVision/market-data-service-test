"""
V2 USStock indicator tests (10 indicators)
MA, EMA, MACD, KDJ, RSI, BOLL, ATR, OBV, CCI, ADX
Endpoints: /api/v1/usstock/{indicator}
"""
...
def test_usstock_single_value_indicators(tester: BaseTester) -> list:
    """Test US stock single-value indicators: MA, EMA, RSI, ATR, OBV, CCI"""
    results = []

    for symbol in US_SYMBOLS:
        tester._log(f"\n--- Testing US single-value indicators for {symbol} ---")
        ...
        for indicator, cfg in SINGLE_VALUE_INDICATORS.items():
            ...
            results.append(tester._make_result(
                f"US Indicator: {symbol}/{indicator}",
                "usstock_indicator_single",
                TestStatus.PASSED if ok else TestStatus.FAILED,
                f"{symbol}/{indicator}" if ok else f"{symbol}/{indicator}: missing fields",
...
def test_usstock_multi_value_indicators(tester: BaseTester) -> list:
    """Test US stock multi-value indicators: MACD, KDJ, BOLL, ADX"""
    results = []

    for symbol in US_SYMBOLS:
        tester._log(f"\n--- Testing US multi-value indicators for {symbol} ---")
