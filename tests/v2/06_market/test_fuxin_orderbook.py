...
def test_fuxin_orderbook(tester: BaseTester) -> list:
    """十档买卖盘接口完整测试"""
    results = []
    ...
    for label, market, code, display in test_cases:
        name = f"十档买卖盘: {label} - {display} ({code})"
        resp = tester.client.v2_fuxin_orderbook(market, code)
        body = _get_body(resp)
        ...
        if ok and isinstance(bid, list) and isinstance(ask, list) and (len(bid) > 0 or len(ask) > 0):
            ...
            results.append(tester._make_result(
                name, "fuxin_orderbook", TestStatus.PASSED if ok else TestStatus.FAILED,
            ...
            results.append(tester._make_result(
                name, "fuxin_orderbook", TestStatus.SKIPPED,
                f"空数据（非交易时段/停牌）: {msg}",
                resp.response_time_ms,
            ))
        else:
            results.append(tester._make_result(
                name, "fuxin_orderbook", TestStatus.FAILED,
                f"请求失败: HTTP {resp.status_code}",
                resp.response_time_ms, resp.error,
            ))

    # 2. CN 带前缀 (sh600519)
    name = "十档买卖盘: CN - 带前缀 sh600519"
    resp = tester.client.v2_fuxin_orderbook("cnstock", "sh600519")
    body = _get_body(resp)
    ok = resp.success and body.get("success") is True and body.get("symbol") == "600519"
    results.append(tester._make_result(
        name, "fuxin_orderbook",
    ...
    # 3. CN 带后缀 (600519.SH)
    name = "十档买卖盘: CN - 带后缀 600519.SH"
    resp = tester.client.v2_fuxin_orderbook("cnstock", "600519.SH")
    body = _get_body(resp)
    ok = resp.success and body.get("success") is True and body.get("symbol") == "600519"
    results.append(tester._make_result(
        name, "fuxin_orderbook",
